...
public class Logs {
	...
	public static void initialize(ILogConfig config) {
		if (config == null) {
			throw new Error(I18N.formatMessage(YMP.__LSTRING_FILE, null, null, "ymp.commons.logger_module_init_parameter_null"));
		}
		__LOG_CONFIG = config;
	...
	public static ILogger create(LogLevel defaultLevel, String loggerName) {
		...
		if (logger == null) {
			...
			if (logger != null) {
				...
				throw new Error(I18N.formatMessage(YMP.__LSTRING_FILE, null, null, "ymp.commons.logger_name_invalid", loggerName));
			}
		} else if (loggerName.equals(ILogConfig.DEFAULT_LOGGER_NAME)) {
			throw new Error(I18N.formatMessage(YMP.__LSTRING_FILE, null, null, "ymp.commons.logger_name_protected", loggerName));
		} else {
	...
	public static ILogger get(String loggerName) {
		...
		if (logger == null) {
			throw new Error(I18N.formatMessage(YMP.__LSTRING_FILE, null, null, "ymp.commons.logger_not_exist", loggerName));
		}
		return logger;
	}
